...
contract('Datastore ', accounts => {
    ...
    beforeEach(async () => {
        ...
        await datastore.addDatastoreOwner(root, { from: holder })

    })

    
    it("doesn't allow file rename without write access", async () => {
        const groupId = await datastore.createGroup('GROUP1', { from: holder })
        
        await datastore.addEntityToGroup(1, testAccount, { from: holder })
        
        await datastore.addFile('ref', 'filename.jpg', 123, true, { from: holder })
        
        // Write access set to true
        await datastore.setGroupPermissions(1, 1, true, false, { from: holder })
        
        await assertThrow(async () => {
            await datastore.setFileName(1, 'new filesname.jpg', { from: testAccount })   
        })

    })

    it('allows file rename with group write access', async () => {
        const groupId = await datastore.createGroup('GROUP1', { from: holder })
        ...
        // Write access set to true
        await datastore.setGroupPermissions(1, 1, true, true, { from: holder })
    ...
    xit('deleteGroup deletes a group', async () => {
        await datastore.createGroup('My first group')
        await datastore.createGroup('My second group')
    ...
    xit('renameGroup renames an existing group', async() => {
